...
if latest_image:
    ...
    # 测试保存工艺选择API - 包含倒角和centre drilling
    test_selections = [
        # 槽特征（F1, F2）
        {
            'feature_name': 'F1',
        ...
        # 孔特征（F4）- centre drilling, drilling, milling
        {
            'feature_name': 'F4',
    ...
    if get_response.status_code == 200:
        ...
        print()
        print('Saved selections:')
        for selection in result['selections']:
            chamfer_mark = ' [倒角第二工序]' if selection['is_chamfer_second'] else ''
            print(f"  {selection['feature_name']}[{selection['sequence']}] - {selection['operation']}{chamfer_mark}:")
